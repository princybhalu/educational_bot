...
// @ts-ignore
type TaskStatus = 'pending' | 'complete' | 'inprogress';
...
const TaskCard3: React.FC<{ task: Task2 }> = ({ task }) => {
  ...
  return (
    <div
      className={`relative bg-white border-l-4 ${getBorderColor(task.status)} p-4 rounded-lg shadow-md mb-6`}
    >
      ...
      {task.meta_data.topic && (
        <p className="text-sm text-gray-500">Topic: {task.meta_data.topic}</p>
      )}
    </div>
  );
};

const TaskTimeline: React.FC<{ tasks: Task2[] }> = ({ tasks }) => (
